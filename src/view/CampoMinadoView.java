...
public class CampoMinadoView extends JFrame {
    ...
    private static final String[] TEMAS_FUNDO = {"Escuro", "Claro", "Campo", "Terminal retrô", "Cyberpunk neon", "Halloween", "Oceano tropical", "Alto contraste", "Automático"};
    ...
    private JCheckBox chkSemCascata, chkMinasVisiveis, chkRelampago, chkToroidal, chkPergunta;
    private int zoom = 36;
    ...
    public void mostrarTelaInicial() {
        ...
        JButton btnPersonalizado = new JButton("Criar tabuleiro personalizado");
        btnPersonalizado.setFont(FONTE_PEQUENA);
        btnPersonalizado.addActionListener(e -> pedirTabuleiroPersonalizado());
    ...
    private JPanel criarPainelOpcoes() {
        ...
        JPanel modos = new JPanel(new GridLayout(2, 3, 4, 4));
        modos.setBackground(COR_FUNDO);
        chkSemCascata = criarCheck("Sem cascata");
        chkMinasVisiveis = criarCheck("Minas visíveis");
        chkRelampago = criarCheck("Relâmpago");
        chkToroidal = criarCheck("Bordas conectadas");
        chkPergunta = criarCheck("Marcação ?");
        modos.add(chkSemCascata); modos.add(chkMinasVisiveis); modos.add(chkRelampago);
        modos.add(chkToroidal); modos.add(chkPergunta);
        painel.add(modos);
        painel.add(Box.createVerticalStrut(10));
        ...
        JButton btnRetomar = new JButton("Retomar partida salva");
        btnRetomar.addActionListener(e -> { if (ouvinte != null) ouvinte.aoRetomarPartida(); });
    ...
    private JCheckBox criarCheck(String texto) {
        ...
        c.setFont(FONTE_PEQUENA); c.setForeground(COR_TEXTO_SECUNDARIO); c.setBackground(COR_FUNDO);
        c.setFocusPainted(false); return c;
    }

    private void pedirTabuleiroPersonalizado() {
        try {
            int linhas = Integer.parseInt(JOptionPane.showInputDialog(this, "Linhas (2-30):", "9"));
            int colunas = Integer.parseInt(JOptionPane.showInputDialog(this, "Colunas (2-40):", "9"));
            int minas = Integer.parseInt(JOptionPane.showInputDialog(this, "Minas:", "10"));
            if (ouvinte != null && linhas > 0 && colunas > 0 && minas >= 0 && minas < linhas * colunas) ouvinte.aoEscolherDificuldade(linhas, colunas, minas);
        } catch (NumberFormatException ignored) { mostrarAviso("Informe números válidos."); }
    }
    ...
    public void aplicarTemaSelecionado() {
        if (comboTemaFundo != null) {
            ...
            if ("Claro".equals(tema)) {
                ...
                corFundo = new Color(8, 18, 8); corFundoClaro = new Color(14, 30, 14);
                corTextoPrincipal = new Color(120, 255, 120); corTextoSecundario = new Color(70, 190, 70);
                corCard = new Color(10, 25, 10); corCardHover = new Color(20, 50, 20); corDestaque = new Color(120, 255, 120); corBorda = new Color(40, 120, 40);
            } else if ("Cyberpunk neon".equals(tema)) {
                corFundo = new Color(20, 8, 35); corFundoClaro = new Color(38, 12, 58);
                corTextoPrincipal = new Color(240, 230, 255); corTextoSecundario = new Color(180, 140, 220); corCard = new Color(35, 10, 52); corCardHover = new Color(60, 15, 80); corDestaque = new Color(0, 240, 255); corBorda = new Color(220, 20, 180);
            } else if ("Halloween".equals(tema)) {
                corFundo = new Color(35, 15, 10); corFundoClaro = new Color(60, 25, 15); corTextoPrincipal = new Color(255, 220, 150); corTextoSecundario = new Color(220, 150, 90); corCard = new Color(48, 18, 28); corCardHover = new Color(75, 25, 35); corDestaque = new Color(255, 130, 20); corBorda = new Color(110, 55, 30);
            } else if ("Oceano tropical".equals(tema)) {
                corFundo = new Color(5, 35, 60); corFundoClaro = new Color(8, 55, 80); corTextoPrincipal = new Color(220, 250, 255); corTextoSecundario = new Color(130, 215, 220); corCard = new Color(8, 65, 82); corCardHover = new Color(15, 90, 100); corDestaque = new Color(255, 125, 100); corBorda = new Color(20, 125, 140);
            } else if ("Alto contraste".equals(tema)) {
                corFundo = Color.BLACK; corFundoClaro = new Color(20, 20, 20); corTextoPrincipal = Color.WHITE; corTextoSecundario = Color.YELLOW; corCard = Color.BLACK; corCardHover = new Color(50, 50, 50); corDestaque = Color.YELLOW; corBorda = Color.WHITE;
            } else if ("Automático".equals(tema)) {
                boolean claro = !UIManager.getDefaults().getBoolean("darkMode");
                corFundo = claro ? new Color(245,245,250) : COR_FUNDO; corFundoClaro = claro ? new Color(230,230,235) : COR_FUNDO_CLARO;
                corTextoPrincipal = claro ? new Color(25,25,30) : COR_TEXTO_PRINCIPAL; corTextoSecundario = claro ? new Color(95,95,110) : COR_TEXTO_SECUNDARIO;
            } else {
                corFundo = COR_FUNDO;
    ...
    private JPanel criarPainelSuperior() {
        ...
        JPanel acoes = new JPanel(new FlowLayout(FlowLayout.LEFT, 4, 0));
        acoes.setOpaque(false);
        acoes.add(btnNovo);
        JButton btnDica = new JButton("Dica");
        btnDica.addActionListener(e -> { if (ouvinte != null) ouvinte.aoPedirDica(); });
        JButton btnSalvar = new JButton("Salvar");
        btnSalvar.addActionListener(e -> { if (ouvinte != null) ouvinte.aoSalvarPartida(); });
        JButton btnExportar = new JButton("CSV");
        btnExportar.addActionListener(e -> { if (ouvinte != null) ouvinte.aoExportarEstatisticas(); });
        JButton btnTelaCheia = new JButton("Tela cheia");
        btnTelaCheia.addActionListener(e -> { if (ouvinte != null) ouvinte.aoAlternarModo("tela-cheia"); });
        acoes.add(btnDica); acoes.add(btnSalvar); acoes.add(btnExportar); acoes.add(btnTelaCheia);
        painel.add(acoes, BorderLayout.WEST);
...
painel.add(Box.createVerticalStrut(15));
        ...
        botao.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent evento) {
                ...
                if (botaoDireito) {
                    ...
                    ouvinte.aoUsarChording(linha, coluna);
    ...
    public void atualizarCelula(int linha, int coluna, LeituraTabuleiro leitura) {
        ...
        if (leitura.isQuestionada(linha, coluna)) {
            botao.setText("?");
        ...
        if (leitura.isMinasVisiveis() && leitura.isMinada(linha, coluna)) {
    ...
    public void atualizarTabuleiro(LeituraTabuleiro leitura) {
        for (int i = 0; i < leitura.getLinhas(); i++) {
            for (int j = 0; j < leitura.getColunas(); j++) {
                atualizarCelula(i, j, leitura);
            }
        }
    }
...
public void mostrarVitoria(String mensagemRecorde) {
    ...
    /** Pisca o fundo e o status por alguns ciclos, sem bloquear a EDT. */
    public void iniciarAnimacaoExplosao() {
        final int[] ciclo = {0};
        Timer timer = new Timer(120, e -> {
            ciclo[0]++;
            boolean ativo = ciclo[0] % 2 == 1;
            piscarFundoDeExplosao(ativo);
            labelStatus.setText(ativo ? EMOJI_EXPLOSAO + " BOOM!" : EMOJI_EXPLOSAO + " Você perdeu!");
            if (ciclo[0] >= 10) {
                ((Timer) e.getSource()).stop();
                piscarFundoDeExplosao(false);
                mostrarDerrota();
            }
        });
        timer.start();
    ...
    /** Alterna confetes no texto e destaca as células reveladas como feedback de vitória. */
    public void iniciarAnimacaoVitoria() {
        final int[] ciclo = {0};
        Timer timer = new Timer(140, e -> {
            ciclo[0]++;
            String prefixo = ciclo[0] % 2 == 0 ? "✦  " : "✧  ";
            labelStatus.setText(prefixo + EMOJI_TROFEU + " VITÓRIA! " + prefixo);
            labelStatus.setForeground(ciclo[0] % 2 == 0 ? new Color(255, 215, 0) : COR_VITORIA);
            if (botoes != null) {
                for (JButton[] linha : botoes) for (JButton botao : linha)
                    if (Boolean.TRUE.equals(botao.getClientProperty("revelada")))
                        botao.setBackground(ciclo[0] % 2 == 0 ? new Color(45, 130, 70) : new Color(40, 100, 60));
            }
            if (ciclo[0] >= 12) { ((Timer) e.getSource()).stop(); mostrarVitoria(); }
        });
        timer.start();
    }

    public boolean isModoSemCascata() { return chkSemCascata != null && chkSemCascata.isSelected(); }
    public boolean isModoMinasVisiveis() { return chkMinasVisiveis != null && chkMinasVisiveis.isSelected(); }
    public boolean isModoRelampago() { return chkRelampago != null && chkRelampago.isSelected(); }
    public boolean isModoToroidal() { return chkToroidal != null && chkToroidal.isSelected(); }
    public boolean isModoPergunta() { return chkPergunta != null && chkPergunta.isSelected(); }
    public void mostrarAviso(String texto) { JOptionPane.showMessageDialog(this, texto, "Campo Minado", JOptionPane.INFORMATION_MESSAGE); }
    public void alternarTelaCheia() { setExtendedState(getExtendedState() == JFrame.MAXIMIZED_BOTH ? JFrame.NORMAL : JFrame.MAXIMIZED_BOTH); }
    public void aumentarZoom() { zoom = Math.min(64, zoom + 4); atualizarZoom(); }
    public void diminuirZoom() { zoom = Math.max(20, zoom - 4); atualizarZoom(); }
    private void atualizarZoom() { if (botoes != null) for (JButton[] linha : botoes) for (JButton b : linha) b.setPreferredSize(new Dimension(zoom, zoom)); pack(); }

    /** Atalhos: setas movem o foco, espaço revela, F marca, H pede dica, +/- ajustam zoom. */
    public void instalarAtalhos() {
        if (botoes == null) return;
        for (JButton[] linha : botoes) for (JButton b : linha) {
            b.getInputMap(JComponent.WHEN_FOCUSED).put(KeyStroke.getKeyStroke("SPACE"), "revelar");
            b.getActionMap().put("revelar", new AbstractAction() { public void actionPerformed(java.awt.event.ActionEvent e) { b.doClick(); } });
            b.getAccessibleContext().setAccessibleName("Célula do Campo Minado");
        }
    }
}
